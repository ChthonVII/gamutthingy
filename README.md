...
- `--nesagcluma`: When generating NES palette, type of CRT automatic gain control to use for luma. Possible values are `nominal`(default) to assume nominal IRE value of 1/140 volts, `sync` to normalize to the difference between blanking voltage and sync voltage, and `burst` to normalize to the color burst amplitude. The resulting white levels are 110 IRE, 119 IRE, and 84 IRE, respectively. For `nominal` and `sync`, `--nessuperwhites` controls how the out-of-bounds white is handled. For `burst`, output values are renormalized after linearization to put white at 1.0. (In this case, `--crtwhite` corresponds to 100 IRE rather than NES's white.) The behavior for `nominal` is the same as gamutthingy's old behavior, and the same as palgen_persune's behavior.
...
- `--nessuperwhites`: When generating NES palette, specifies whether to show "super white" colors resulting from `--nesagcluma` set to `nominal` or `sync`. Possible values are `true`(default) and `false`. If `true`, gamma-space colors are renormalized so that NES's white is 1.0. If `--crtblackpedestalcrush` is also `true`, then `--crtblackpedestalcrush-amount` is rescaled accordingly. (In this case, `--crtwhite` corresponds to NES's white rather than 100 IRE.) If `false`, luma is clipped to 100 IRE, and out-of-bounds chroma (if any) is resolved by the gamut compression algorithm. The behavior for `true` is the same as gamutthingy's old behavior (except for rescaling the black pedestal), and the same as palgen_persune's behavior.
