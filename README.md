...
- `--nesboost48c`: When generating NES palette, luma boost, in nominal IRE units, caused by design of the traces for hues \#4, \#8, and \#C. Floating point number. Default 1.0.
...
- `--nesagcluma`: When generating NES palette, type of CRT automatic gain control to use for luma. Possible values are `nominal`(default) to assume nominal IRE value of 1/140 volts, `sync` to normalize to the difference between blanking voltage and sync voltage, `burst` to normalize to the color burst amplitude, and `adaptive` to normalize to detected white inputs. The resulting levels for NES's $30 white are 110 IRE, 119 IRE, 84 IRE, and 100 IRE respectively. For `nominal` and `sync`, `--nessuperwhites` controls how the out-of-bounds white is handled. For `burst`, output values are renormalized after linearization to put white at 1.0. (Note that `--crtwhite` always corresponds to 100 IRE rather than NES's $30 white.) The behavior for `adaptive` simply assumes (possibly incorrectly) that the adaptive gain feature is well designed and that white and black occur frequently enough in the input signal to get consistently correct gain. (In reality, this was often not true. [Example](https://www.youtube.com/watch?v=OpYA6J6MZZM).) The behavior for `adaptive` is the same as gamutthingy's old behavior, and the same as palgen_persune's behavior.
- `--nesagcchroma`: When generating NES palette, type of CRT automatic gain control to use for chroma. Possible values are `same` to use whatever was used for luma, and `burst` (default) to normalize to the color burst amplitude. This parameter replaces the old `--nesburstnorm` parameter.
- `--nessuperwhites`: When generating NES palette, specifies the fraction to be shown of the "super white" range resulting from `--nesagcluma` set to `nominal` or `sync`. Floating point number 0.0 to 1.0 (default). Clips the complementary fraction of (linear) luminosity exceeding 1.0, then renormalizes (linear) colors to bring NES's $30 white down to 1.0. Correct setting depends upon how much space the CRT contrast knob position leaves between 100 IRE white and the maximum displayable white. Note that `--crtwhite` always corresponds to 100 IRE rather than NES's $30 white. So it is often appropriate to reduce `--crtwhite` to simulate a lower contrast knob setting when `--nessuperwhites` is greater than 0. (For example, to accommodate `--nessuperwhites` of 1.0, `--crtwhite` should not exceed roughly 80% of maximum displayable white if `--nesagcluma` is `nominal`, or roughly 66% of maximum displayable white if `--nesagcluma` is `sync`.) The behavior for 1.0 is almost the same as gamutthingy's old behavior, and almost the same as palgen_persune's behavior (except the renormalization is now done in linear space).
...
- `--crtdemod`: Specifies the composite-to-R'G'B' demodulator chip in the CRT television. CRT "color correction" is accomplished via the demodulation angles and gains built into this chip. Possible values are:
     ...
     - `TA8867AN` Used in Toshiba CE-20D10 (NTSC, launched 1994, manufactured 1995). Might possibly pair with `P22_ebuish` phosphors (dubious).
