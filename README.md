...
- `--nesagcluma`: When generating NES palette, type of CRT automatic gain control to use for luma. Possible values are `nominal`(default) to assume nominal IRE value of 1/140 volts, `sync` to normalize to the difference between blanking voltage and sync voltage, and `burst` to normalize to the color burst amplitude. The resulting levels for NES's $30 white are 110 IRE, 119 IRE, and 84 IRE, respectively. For `nominal` and `sync`, `--nessuperwhites` controls how the out-of-bounds white is handled. For `burst`, output values are renormalized after linearization to put white at 1.0. (Note that `--crtwhite` always corresponds to 100 IRE rather than NES's $30 white.) The behavior for `nominal` is the same as gamutthingy's old behavior, and the same as palgen_persune's behavior.
